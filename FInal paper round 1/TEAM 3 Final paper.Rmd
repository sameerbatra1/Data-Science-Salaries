...
- [4. Data Preprocessing for Modeling (Final part)](#4.DataPreprocessing-for-Modeling-(Finalpart))
- [5. Regression Models for Salary Prediction](#5.Regression-Models-for-Salary-Prediction)
- [6. Classification Models for Income Categories](#6.Classification-Models-for-Income-Categories)
...
## 3.1 Loading Dataset
The dataset used for this project was sourced from Kaggle’s publicly available *Data Science Salaries Dataset*. It initially contained 6,599 observations covering five years (2020–2024) and spanned a broad range of job titles and employment models across different countries.

Before performing exploratory analysis and modeling, we conducted a thorough data cleaning process to ensure quality, consistency, and relevance for our salary prediction objectives.
...
### 3.2.1 Basic Inspections
Basic inspections were conducted to verify:

- Presence of missing data
- Variable types (factors, integers, characters)
...
```
**Result:**  
- No missing values were detected (`colSums(is.na())` returned 0 for all columns).
- Variables included `job_title`, `experience_level`, `employment_type`, `work_models`, `company_size`, `work_year`, `salary_in_usd`, `salary_currency`, and `employee_residence`.



### 3.2.2 Data Filtering: U.S.-Based Employees Only
...
#### Data Filtering & Remove unnecessary columns
```{r include=TRUE, echo=FALSE, results='markup'}
# Filter data(US)
...
```
We filtered the dataset to retain only U.S.-based employees to ensure salary comparability and minimize exchange rate effects.



### 3.2.3 Outlier Detection and Removal
To minimize the influence of extreme salary outliers, we used the Interquartile Range (IQR) method:
...
- Approximately 101 outliers were removed.
- Final dataset size after outlier removal: **5,204 observations**.
...
We analyzed the average salary by work year (2020–2024).
...
Key findings:
- A steady upward salary trend was observed from 2020 to 2023.
- The peak average salary occurred in **2023**, around **\$156,000**.
- A slight decrease was observed in **2024**.


A one-way ANOVA test showed significant differences across years:
```{r include=TRUE, echo=FALSE, results='markup'}
# ANOVA test
...
```
Result:  
- F-statistic = **6.716**, p-value = **2.18e-05** (highly significant)



### 3.3.2 Salary by Experience Level
We examined salary differences across experience levels: Entry-level, Mid-level, Senior-level, and Executive-level.
...
Key findings:
- Salaries rise sharply with increasing experience.
- Executive-level roles had salaries exceeding **\$250,000**.


ANOVA results confirmed significant salary differences:
```{r include=TRUE, echo=FALSE, results='markup'}
# ANOVA test
...
```
Result:  
- F-statistic = **249.2**, p-value < **2e-16** (extremely significant)



### 3.3.3 Salary by Work Model
We compared salaries across work models: Remote, On-site, and Hybrid.
...
```
Key findings:
- Remote positions paid the highest salaries.
- Hybrid roles showed greater variability.
...
```
Result:  
- F-statistic = **4.969**, p-value = **0.00698** (statistically significant)


### 3.3.4 Salary by Company Size
We also explored salary differences by company size (Small, Medium, Large).
...
```
Result:  
- F-statistic = **8.18**, p-value = **0.000284**
...
While EDA provides descriptive insights, statistical hypothesis testing is essential for assessing whether observed differences are statistically significant and not due to random chance. In this section, we applied normality tests, ANOVA, t-tests, and chi-squared tests.


### 3.4.1 Normality Testing: Shapiro-Wilk Tests
Before applying parametric tests, we tested for normality using the Shapiro-Wilk test.
...
# **4. Data Preprocessing for Modeling (Final part)**
After completing data exploration and statistical testing, we built predictive models to estimate salaries and classify salary ranges. 
Two major modeling approaches were used:

- **Regression Models** for predicting continuous salary (`salary_in_usd`)
- **Classification Models** for categorizing salaries into income brackets
Our goal was to evaluate model accuracy, performance, and reliability based on the available features.
...
## 4.1 Encoding
Before modeling, categorical variables were properly encoded as factors before modeling.
...
## 4.2 Building Train/Test Dataset
```{r include=TRUE, echo=FALSE, results='markup'}
...
# **5. Regression Models for Salary Prediction**
...
# **6.Classification Models for Income Categories**
## 6.1 Converting dataset to classfication
```{r include=TRUE, echo=FALSE, results='hide'}
...
## 6.2 Building Train/Test Dataset
```{r include=TRUE, echo=FALSE, results='markup'}
...
### 6.5.1 XGBoost Hyperparameter Tuning 
```{r include=TRUE, echo=FALSE, results='hide'}
...
## 6.7 Support Vector Machine (SVM) Classifier
```{r include=TRUE, echo=FALSE, results='markup'}
...
## 6.8 Predictions and Example Results
```{r include=TRUE, echo=FALSE, results='markup'}
...
| Model                        | Accuracy (%) |
|-------------------------------|--------------|
| **SVC (Support Vector Classifier)** | **74.74** |
| Random Forest Classifier      | 74.54 |
| XGBoost Classifier            | 74.13 |
| Multinomial Logistic Regression | 73.10 |
| Naive Bayes Classifier        | 73.10 |

**Results:**  
The SVC model was the best performer for predicting income categories.



# **7. Conclusion**
## **7.1 Predictive Modeling Insights**
This project extends the midterm analysis by integrating predictive modeling and classification techniques to provide a deeper understanding of salary trends in the data science field from 2020 to 2024. Through regression models, it became evident that predicting exact salary values remains challenging due to the complexity and variability of influencing factors. Ensemble methods like Random Forest and XGBoost offered slight improvements over linear models but still faced limitations in explanatory power.

In contrast, classification models demonstrated higher reliability when categorizing salaries into broader income ranges. Support Vector Classifiers(SVC) and Random Forest Classifiers, in particular, achieved the best performance, suggesting that advanced modeling techniques are well-suited to capturing salary tier patterns. The most influential factors across models were experience level and job title, reaffirming their critical roles in compensation decisions.


## **7.2 Limitations**
- **Feature Limitations:**  
  Important factors such as education, technical skills, certifications, and company reputation were not available in the dataset.
  
- **Moderate Variance Explanation:**  
  Even the best models left significant salary variability unexplained, suggesting the need for richer feature sets.

- **Self-Reported Data Bias:**  
  Potential biases in self-reported salary data may affect generalizability.


## **7.3 Recommendations for Future Work**
- Incorporate additional features such as educational background, technical certifications, skillsets, and company rankings.
- Explore advanced modeling techniques (e.g., stacking, deep learning) to improve predictive performance.
- Expand analysis to international datasets to compare global salary trends.
- Investigate longitudinal effects using time series or panel data approaches.

## **7.4 Final Thoughts **
These findings offer practical insights for both job seekers and employers: while precise salary predictions may be uncertain, identifying salary tiers based on candidate characteristics is a more attainable goal.
...
1. Kaggle. (n.d.). *Data Science Salaries Dataset*. Retrieved from [https://www.kaggle.com/datasets/ruchi798/data-science-job-salaries](https://www.kaggle.com/datasets/ruchi798/data-science-job-salaries)

2. U.S. Bureau of Labor Statistics. (2022). *Occupational Outlook Handbook: Data Scientists*. Retrieved from [https://www.bls.gov/ooh/math/data-scientists.htm](https://www.bls.gov/ooh/math/data-scientists.htm)
