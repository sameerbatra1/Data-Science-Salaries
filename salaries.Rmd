...
df$predicted_salary <- predict(model_lm, newdata = df)

# RMSE Calculation
rmse <- sqrt(mean((df$salary_in_usd - df$predicted_salary)^2))
cat("Linear regression RMSE: ", round(rmse, 2), "\n")

# Plot actual vs predicted salaries
ggplot(df, aes(x = salary_in_usd, y = predicted_salary)) +
...
set.seed(123)
train_index <- createDataPartition(df_matrix$salary_in_usd, p = 0.8, list = FALSE)
train_data <- df_matrix[train_index, ]
test_data <- df_matrix[-train_index, ]

# Prepare data for XGBoost
dtrain <- xgb.DMatrix(data = as.matrix(train_data[,-1]), label = train_data$salary_in_usd)
dtest <- xgb.DMatrix(data = as.matrix(test_data[,-1]), label = test_data$salary_in_usd)
...
# RMSE
rmse <- sqrt(mean((xgb_preds - test_data$salary_in_usd)^2))
cat("XGBoost Regression RMSE:", round(rmse, 2), "\n")

# Plot actual vs predicted
ggplot(data.frame(actual = test_data$salary_in_usd, predicted = xgb_preds),
