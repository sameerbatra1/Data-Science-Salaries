...
original_salaries_train <- test_df$salary_in_usd
original_salaries_test <- test_df$salary_in_usd

# Remove salary_in_usd from train_df and test_df
train_df <- train_df %>%
...
```{r include=TRUE, echo=TRUE, results='markup'}
X_train <- train_df[, -which(names(train_df) == "salary_in_usd")]
y_train <- train_df$salary_in_usd
X_test <- test_df[, -which(names(test_df) == "salary_in_usd")]
...
# Train linear regression on scaled salary
model_lm <- lm(salary_scaled ~ job_title + experience_level + employment_type + work_models + work_year + company_size,
               data = train_df)

# Predict on test set (scaled salary)
test_df$predicted_salary_scaled <- predict(model_lm, newdata = test_df)

# Calculate RMSE on scaled salary
rmse <- sqrt(mean((test_df$salary_scaled - test_df$predicted_salary_scaled)^2))
cat("Linear regression RMSE (scaled):", round(rmse, 4), "\n")

# Plot actual vs predicted (scaled)
ggplot(test_df, aes(x = salary_scaled, y = predicted_salary_scaled)) +
  geom_point(alpha = 0.5, color = "green") +
  geom_abline(color = "blue", linetype = "dashed") +
  labs(
    title = "Actual vs Predicted Scaled Salary (Linear Regression)",
    x = "Actual Scaled Salary",
    y = "Predicted Scaled Salary"
  ) +
  theme_minimal()
...
```
```{r}
# write me code to remove company location from train_df
# Remove company location from train_df
train_df <- train_df %>%
...
# Convert categorical variables to integer for train and test
train_matrix <- train_df %>%
  mutate(across(c(job_title, employment_type, work_models, company_size, experience_level), as.integer)) %>%
  select(salary_scaled, job_title, employment_type, work_models, company_size, experience_level, work_year)
...
X_train <- train_matrix %>% select(-salary_scaled)
y_train <- train_matrix$salary_scaled

X_test <- test_matrix %>% select(-salary_scaled)
y_test <- test_matrix$salary_scaled
...
# Calculate RMSE
rmse <- sqrt(mean((xgb_preds - y_test)^2))
cat("XGBoost Regression RMSE:", round(rmse, 2), "\n")
...
rf_model <- randomForest(
  salary_scaled ~  job_title + experience_level + employment_type + work_models + work_year + company_size,
  data = train_df,
  ntree = 500,       
  importance = TRUE
)
...
#Predict on test set (scaled salary)
rf_pred_scaled <- predict(rf_model, newdata = test_df)
head(rf_pred_scaled)
# convert rf_pred back to original scale  
rf_pred <- rf_pred_scaled * (salary_max - salary_min) + salary_min
head(rf_pred)
```
```{r}
library(ggplot2)

ggplot(data.frame(Actual = test_df$salary_in_usd, Predicted = rf_pred),
       aes(x = Actual, y = Predicted)) +
  geom_point(alpha = 0.5, color = "steelblue") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Random Forest: Predicted vs Actual Salary",
       x = "Actual Salary (USD)",
...
# Model training (SVM regression training)
```{r}
set.seed(42)
svm_model <- svm(
  salary_scaled ~ job_title + employment_type + work_models + company_size + experience_level + work_year,,
...
svm_pred_scaled <- predict(svm_model, newdata = test_df)
head(svm_pred_scaled)
svm_pred <- svm_pred_scaled * (salary_max - salary_min) + salary_min
head(svm_pred)
```

# SVM Prediction vs Actual Salary
```{r}
library(ggplot2)

ggplot(data.frame(Actual = test_df$salary_in_usd, Predicted = svm_pred), 
       aes(x = Actual, y = Predicted)) +
  geom_point(alpha = 0.5, color = "darkgreen") +
  geom_abline(slope = 1, intercept = 0, color = "blue", linetype = "dashed") +
  labs(title = "SVM: Predicted vs Actual Salary",
       x = "Actual Salary (USD)",
       y = "Predicted Salary (USD)") +
  theme_minimal()
...
```
The SVM regression model showed an RMSE of about $49,133, an MAE of about $38,736, and an R² of 0.2937, showing overall better performance than Random Forest.
